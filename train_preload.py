...
from dataset_preload import DataLoader, PreLoadDataset, collate_fn_tensor, pad_2D_tensor
...
def load_data_to_gpu(batch, device):
    if False:
        stream = torch.cuda.Stream()
        with torch.cuda.stream(stream):
            mel = batch["mel"].float().to(device)
            wav = batch["wav"].float().to(device)
            mel = mel.contiguous().transpose(1, 2)
            weight_index = batch["weight"]
            weight_start_index = batch["weight_start_index"]
            weight_end_index = batch["weight_end_index"]
            weight_list = [torch.from_numpy(np.load(index)).transpose(0, 1)[weight_start_index[i]:weight_end_index[i], :] for i, index in enumerate(weight_index)]
            weight = pad_2D_tensor(weight_list)
            weight = weight.float().to(device)
            return {"mel": mel, "wav": wav, "weight": weight}
    else:
        mel = batch["mel"].float().to(device)
        wav = batch["wav"].float().to(device)
        mel = mel.contiguous().transpose(1, 2)
...
def main(args):
    ...
    model = MelGANGenerator(basis_signal_weight).to(device)
        ...
        for i, batchs in enumerate(training_loader):
            ...
            for j, db in enumerate(batchs):
                ...
                if preload_data == None:
                    data_of_batch = load_data_to_gpu(db, device)
                    mel = data_of_batch["mel"]
                    weight = data_of_batch["weight"]
                    wav = data_of_batch["wav"]
                else:
                ...
                if False:
                    # Serial
                    if j + 1 != number_of_batch:
                        preload_data = load_data_to_gpu(batchs[j + 1], device)
                    else:
                        preload_data = None
                    ...
                    if j + 1 != number_of_batch:
                        futures = [
                            executor.submit(partial(load_data_to_gpu, batchs[j + 1], device)),
                        ]
                        time_list = trainer(
                            model, discriminator,
                            optimizer, discriminator_optimizer, basis_signal_optimizer,
